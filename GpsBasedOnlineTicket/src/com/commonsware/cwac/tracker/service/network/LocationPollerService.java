...
package com.commonsware.cwac.tracker.service.network;
...
public class LocationPollerService extends Service {
  ...
  public static void requestLocation(Context ctxt, Intent i) {
    String provider=i.getStringExtra(LocationPollerNetwork.EXTRA_PROVIDER);
    Intent toBroadcast=
        (Intent)i.getExtras().get(LocationPollerNetwork.EXTRA_INTENT);

    if (provider == null) {
      Log.e("newlog", "Invalid Intent -- has no provider");
    }
    else if (toBroadcast == null) {
      Log.e("LocationPoller",
            "newlog Intent -- has no Intent to broadcast");
    }
    else {
      ...
      Log.e("LocationPoller","getLock--"+provider);
    }
  }
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    String provider=
        intent.getStringExtra(LocationPollerNetwork.EXTRA_PROVIDER);
    Intent toBroadcast=
        (Intent)intent.getExtras().get(LocationPollerNetwork.EXTRA_INTENT);
		TIMEOUT = (int) intent.getLongExtra(LocationPollerNetwork.EXTRA_TIMEOUT,
				DEFAULT_TIMEOUT);
  ...
  private class PollerThread extends WakefulThread {
    ...
    private LocationListener listener=new LocationListener() {
      ...
      public void onLocationChanged(Location location) {
        ...
        toBroadcast.putExtra(LocationPollerNetwork.EXTRA_LOCATION, location);
    ...
    protected void onPreExecute() {
			...
			if (!locMgr.isProviderEnabled(provider)) {
				...
				Intent toBroadcast = new Intent(intentTemplate);
				toBroadcast.putExtra(LocationPollerNetwork.EXTRA_ERROR,
						"Location Provider disabled!");
				toBroadcast.putExtra(
						LocationPollerNetwork.EXTRA_ERROR_PROVIDER_DISABLED, true);
				toBroadcast.putExtra(LocationPollerNetwork.EXTRA_LASTKNOWN,
      ...
      onTimeout=new Runnable() {
        public void run() {
          ...
          toBroadcast.putExtra(LocationPollerNetwork.EXTRA_ERROR, "Timeout!");
          toBroadcast.putExtra(
					LocationPollerNetwork.EXTRA_ERROR_PROVIDER_DISABLED, false);
          toBroadcast.putExtra(LocationPollerNetwork.EXTRA_LASTKNOWN,
                               locMgr.getLastKnownLocation(provider));
          sendBroadcast(toBroadcast);
          quit();
